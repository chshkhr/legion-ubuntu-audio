# Ubuntu 26.04 encrypted-root and dracut guide

This guide focuses on the Ubuntu-specific encrypted-root and initramfs parts of
running an AW88399-capable custom kernel. It does not repeat general kernel
compilation, firmware, NVIDIA DKMS, or package-installation instructions; use
the projects linked from [README.md](README.md) for that background.
...
The AW88399 HDA side-codec changes are merged upstream and are expected to
appear in Linux 7.3. Verify the contents and behavior of the released kernel
before removing a known-good custom kernel. The firmware remains required even
when the kernel contains the driver.
